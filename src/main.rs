...
fn concurrencyThread() {
    pub struct Bank {
        balance: f32
    }

    fn withdraw(theBank: &Arc<Mutex<Bank>>, amt: f32) {
        let mut bankRef = theBank.lock().unwrap();
        
        if bankRef.balance <= 5.00 {
            println!("Current Balance: {} Withdraw a smaller amount!", bankRef.balance);
        } else {
            println!("Customer withdrew {}!\nCurrent Balance: {}", amt, bankRef.balance);
            return bankRef.balance -= amt;
        }
    }

    fn customer(theBank: Arc<Mutex<Bank>>) {
        withdraw(&theBank, 5.00);
    }

    let bank: Arc<Mutex<Bank>> = Arc::new(Mutex::new(Bank {
        balance: 20.00
    }));
